...
def _load_card_bg() -> Image.Image:
    ...
    if bg_path.exists():
        ...
        raw = Image.new("RGB", (W, H), (0, 0, 0))

    # If card_bg is full 1536x1024, crop exactly. If not, fit to card area.
    bg = ImageOps.fit(raw, (CARD_W, CARD_H), method=Image.Resampling.LANCZOS, centering=(0.50, 0.48))
...
def _draw_reflection(canvas: Image.Image, card: Image.Image):
    try:
        ...
        ref = ImageOps.flip(ref).filter(ImageFilter.GaussianBlur(4))
        fade = Image.new("L", (CARD_W, rh), 0)
        fd = ImageDraw.Draw(fade)
        for y in range(rh):
            fd.line((0, y, CARD_W, y), fill=max(0, int(40 * (1 - y / rh))))
...
def generate_trade_card(contract_data: Dict[str, Any], current_price: Any = None, status: str = "OPEN") -> str:
    ...
    # Text-readability overlay, keeps robot as watermark.
    card = Image.alpha_composite(bg, Image.new("RGBA", (CARD_W, CARD_H), (0, 0, 0, 70)))

    # Left and bottom vignettes.
    ...
    d.rounded_rectangle((CARD_X1, CARD_Y1, CARD_X2, CARD_Y2), radius=48, outline=(*BORDER, 155), width=3)
    ...
    # Header icons.
    d.rounded_rectangle((ox + 1185, oy + 65, ox + 1252, oy + 132), radius=12, outline=(190, 196, 210, 145), width=2, fill=(20, 22, 28, 90))
    d.text((ox + 1219, oy + 95), "•••", font=f_icon, fill=WHITE, anchor="mm")

    d.rounded_rectangle((ox + 1282, oy + 65, ox + 1351, oy + 132), radius=12, outline=(255, 190, 80, 175), width=2, fill=(155, 94, 22, 230))
