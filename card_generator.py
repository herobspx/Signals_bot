...
# BAM | SPX exact approved style
# Same landscape card ratio as the approved mockup.
W, H = 1536, 1024

CARD_X1, CARD_Y1 = 54, 118
CARD_X2, CARD_Y2 = 1482, 900
...
def _load_card_bg() -> Image.Image:
    ...
    # Important:
    # This crop pushes the robot to the center/right and reduces the heavy ring look.
    bg = ImageOps.fit(
        raw,
        (CARD_W, CARD_H),
    ...
    # Approved card is sharp, not blurred.
    bg = bg.filter(ImageFilter.GaussianBlur(0.05))
    return bg


def _draw_reflection(canvas: Image.Image, card: Image.Image):
    try:
        rh = 125
        ref = card.crop((0, CARD_H - rh, CARD_W, CARD_H))
        ref = ImageOps.flip(ref).filter(ImageFilter.GaussianBlur(3))
        fade = Image.new("L", (CARD_W, rh), 0)
        fd = ImageDraw.Draw(fade)
        for y in range(rh):
            fd.line((0, y, CARD_W, y), fill=max(0, int(44 * (1 - y / rh))))
        canvas.paste(ref, (CARD_X1, CARD_Y2 + 6), fade)
    except Exception:
        pass
...
def generate_trade_card(contract_data: Dict[str, Any], current_price: Any = None, status: str = "OPEN") -> str:
    ...
    # Heavy contrast layer to make the old background behave like watermark.
    card = Image.alpha_composite(bg, Image.new("RGBA", (CARD_W, CARD_H), (0, 0, 0, 150)))

    # Extra left + bottom darkness like approved mockup.
    vignette = Image.new("RGBA", (CARD_W, CARD_H), (0, 0, 0, 0))
    vd = ImageDraw.Draw(vignette)

    for x in range(CARD_W):
        # Slightly darker left side for the big price.
        if x < 760:
            a = int(95 * (1 - x / 760))
    ...
    for y in range(CARD_H):
        if y > 420:
            a = min(125, int((y - 420) / (CARD_H - 420) * 125))
            vd.line((0, y, CARD_W, y), fill=(0, 0, 0, a))

    card = Image.alpha_composite(card, vignette)

    # Paste card
    m = _mask((CARD_W, CARD_H), 48)
    canvas.paste(card, (CARD_X1, CARD_Y1), m)

    _draw_reflection(canvas, card)

    d = ImageDraw.Draw(canvas)

    # Only one light rounded frame, no heavy internal frame.
    d.rounded_rectangle(
    ...
    # Typography tuned larger so Telegram preview is readable.
    f_contract = _font(48, True)
    f_price = _font(160, True)
    f_change = _font(50, True)
    f_time = _font(42, False)
    f_label = _font(43, True)
    ...
    # Header
    d.text((ox + 82, oy + 75), symbol, font=f_contract, fill=WHITE)

    # Header icons
    d.rounded_rectangle(
        (ox + 1185, oy + 65, ox + 1252, oy + 132),
        radius=12,
        outline=(190, 196, 210, 145),
        width=2,
        fill=(20, 22, 28, 90),
    )
    d.text((ox + 1219, oy + 95), "•••", font=f_icon, fill=WHITE, anchor="mm")

    d.rounded_rectangle(
        (ox + 1282, oy + 65, ox + 1351, oy + 132),
        radius=12,
        outline=(255, 190, 80, 175),
        width=2,
        fill=(155, 94, 22, 230),
    )
    d.text((ox + 1317, oy + 98), "⚡", font=_font(42, True), fill=(255, 238, 190), anchor="mm")
    ...
    d.text((change_x, oy + 228), f"{sign}{diff:.2f}", font=f_change, fill=color)
    d.text((change_x, oy + 294), f"{sign}{pct:.2f}%", font=f_change, fill=color)

    # Timestamp
    # Keep ET because your bot is live market-based. Change to MSK only if you explicitly want display text.
    now_et = datetime.now(ET_TZ)
    d.text((ox + 82, oy + 405), f"Open {now_et.strftime('%m/%d %H:%M')} ET", font=f_time, fill=MUTED)

    # Stats: positions match the approved image.
    left_label_x = ox + 82
    left_value_x = ox + 585
    right_label_x = ox + 720
    right_value_x = ox + 1345

    rows_y = [oy + 505, oy + 590, oy + 675]
