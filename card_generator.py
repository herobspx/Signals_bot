...
CARD_X1, CARD_Y1 = 55, 70
CARD_X2, CARD_Y2 = 1225, 650
...
def _fit_bg(path: Path) -> Image.Image:
    ...
    return bg.filter(ImageFilter.GaussianBlur(0.20))
...
def generate_trade_card(
    ...
    dark = Image.new("RGBA", (CARD_W, CARD_H), (0, 0, 0, 120))
    ...
    mask = _rounded_mask((CARD_W, CARD_H), 42)
    ...
    f_contract = _font(38, True)
    f_price = _font(132, True)
    f_change = _font(42, True)
    f_open_ts = _font(32, False)
    f_label = _font(34, True)
    f_val = _font(38, True)
    f_icon = _font(36, True)
    ...
    d.text((ox + 52, oy + 55), symbol, font=f_contract, fill=WHITE)

    d.rounded_rectangle(
        (ox + 965, oy + 55, ox + 1025, oy + 115),
        radius=12,
        outline=(170, 175, 190, 120),
        width=2
    )

    d.text((ox + 995, oy + 82), "•••", font=f_icon, fill=WHITE, anchor="mm")

    d.rounded_rectangle(
        (ox + 1050, oy + 55, ox + 1110, oy + 115),
        radius=12,
        fill=(145, 88, 24, 220),
        outline=(240, 185, 90, 180),
        width=2
    )

    d.text((ox + 1080, oy + 84), "⚡", font=_font(34, True), fill=(255, 236, 180), anchor="mm")

    d.text((ox + 55, oy + 145), _money(shown), font=f_price, fill=color)

    price_w = d.textbbox((0, 0), _money(shown), font=f_price)[2]

    change_x = ox + 55 + price_w + 22

    d.text((change_x, oy + 175), f"{sign}{diff:.2f}", font=f_change, fill=color)
    d.text((change_x, oy + 232), f"{sign}{pct:.2f}%", font=f_change, fill=color)

    now_et = datetime.now(ET_TZ)

    open_ts = f"Open {now_et.strftime('%m/%d %H:%M')} ET"

    d.text((ox + 58, oy + 330), open_ts, font=f_open_ts, fill=MUTED)
    ...
    base.convert("RGB").save(out, "JPEG", quality=97)
