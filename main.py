    ...
    if is_market_open():
                        ...
                        for ev in data:
                            if ev.get("ev") == "T" and ev.get("sym") == ticker:
                                ...
                                if abs(price - t.get("last_price", t["entry"])) >= 0.01:
                                    ...
                                    save_trades()
                                    await send_trade_card(app.bot, PRIVATE_GROUP, t, current_price=price, caption=format_update(t, price))
        ...
        while trade_key in active_trades:
            try:
                ...
                if not active_trades.get(trade_key, {}).get("auto_update", True):
                    await asyncio.sleep(5)
                    continue
...
async def manual_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for k, t in active_trades.items():
            ...
            updated.append(f"{t['symbol']} {t['type']} ${t['strike']}")
        save_trades()
        trades_str = "\n".join(updated) if updated else "لا توجد عقود نشطة"
        await update.message.reply_text(
            f"✅ تم تحديث السعر إلى *${price:.2f}*\n{trades_str}",
            parse_mode="Markdown"
        )
    except ValueError:
