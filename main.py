...
def display_expiry(expiry):
    dt = parse_expiry(str(expiry))
    if not dt:
        return str(expiry)
    return dt.strftime("%d %b %y")

def contract_line(trade):
    opt_type = trade.get("type", "CALL").upper().capitalize()
    return f"{trade.get('symbol','SPXW')} {int(float(trade.get('strike',0)))} {display_expiry(trade.get('expiry',''))} (W) {opt_type}"
...
def _fit_text(draw, text, max_width, start_size, bold=False, min_size=18):
    size = start_size
    while size >= min_size:
...
def make_trade_card(trade, current=None, status="OPEN"):
    """Generate the Webull-style contract image used by the bot."""
    W, H = 1600, 900
    green = (60, 216, 92)
    red   = (255, 76, 88)
    white = (245, 245, 245)
    ...
    if os.path.exists(CARD_BG_FILE):
        bg = Image.open(CARD_BG_FILE).convert("RGB")
        bg = bg.resize((W, H), Image.LANCZOS).filter(ImageFilter.GaussianBlur(3.2))
    else:
    ...
    for x in range(0, 720):
        alpha = int(115 * (1 - x / 720))
        draw.line([(x, 0), (x, H)], fill=(0, 0, 0, alpha))

    draw.line([(85, 220), (45, 260), (85, 300)], fill=white, width=8, joint="curve")

    cline = contract_line(trade)
    cfont = _fit_text(draw, cline, 760, 40, bold=False, min_size=28)
    draw.text((130, 230), cline, font=cfont, fill=white)

    entry = float(trade.get("entry", current or 0) or 0)
    cur = float(current if current is not None else trade.get("last_price", entry) or entry)
    diff = cur - entry
    pct = (diff / entry * 100) if entry else 0
    pos = diff >= 0
    main_color = green if pos else red
    sign = "+" if pos else ""
    arrow = "▲" if pos else "▼"

    price_font = _font(118, bold=True)
    draw.text((90, 365), money(cur), font=price_font, fill=main_color)
    pnl_font = _font(44, bold=False)
    draw.text((95, 545), f"{arrow} {sign}{money(diff)}  {sign}{pct:.2f}%", font=pnl_font, fill=main_color)

    wm_font = _font(92, bold=True)
    wm_text = "BAM | SPX"
    box = draw.textbbox((0,0), wm_text, font=wm_font)
    wx = (W - (box[2]-box[0])) // 2
    wy = 430
    draw.text((wx, wy), wm_text, font=wm_font, fill=(255,255,255,38), stroke_width=1, stroke_fill=(0,0,0,30))

    quote = trade.get("quote", {}) if isinstance(trade.get("quote", {}), dict) else {}
    bid = quote.get("bid") or trade.get("bid") or max(cur - 0.05, 0)
    ask = quote.get("ask") or trade.get("ask") or (cur + 0.05)
    open_p = quote.get("open") or trade.get("open") or entry
    high = quote.get("high") or trade.get("high") or max(cur, entry)
    mid = quote.get("mid") or trade.get("mid") or ((float(bid) + float(ask)) / 2 if bid and ask else cur)
    vol = quote.get("volume") or trade.get("volume") or 0

    x0, y0 = 1165, 190
    col_w, row_h = 170, 180
    for y in [y0 + row_h, y0 + 2*row_h]:
        draw.line([(x0, y), (x0 + 2*col_w + 40, y)], fill=line, width=2)
    for y in [y0, y0 + row_h, y0 + 2*row_h]:
        draw.line([(x0 + col_w, y + 20), (x0 + col_w, y + row_h - 35)], fill=line, width=2)

    label_font = _font(35, bold=False)
    val_font   = _font(40, bold=True)
    small_font = _font(34, bold=False)
    cells = [
        ("Bid", money(bid), "33", green),
        ("Ask", money(ask), "40", red),
        ("Open", money(open_p), "", white),
        ("High", money(high), "", white),
        ("Mid", money(mid), "", white),
        ("Volume", fmt_volume(vol), "", white),
    ]
    for i, (lab, val, sub, color) in enumerate(cells):
        row, col = divmod(i, 2)
        x = x0 + col * col_w + 15
        y = y0 + row * row_h + 20
        draw.text((x, y), lab, font=label_font, fill=white)
        draw.text((x, y + 55), val, font=val_font, fill=color)
        if sub:
            draw.text((x, y + 115), sub, font=small_font, fill=white)
...
async def send_trade_card(bot, chat_id, trade, current=None, caption=None):
    img = make_trade_card(trade, current=current)
    return await bot.send_photo(chat_id=chat_id, photo=img, caption=caption)
                        ...
                        for ev in data:
                            if ev.get("ev") == "T" and ev.get("sym") == ticker:
                                ...
                                if abs(price - t.get("last_price", t["entry"])) >= 0.01:
                                    ...
                                    save_trades()
                                    await send_trade_card(app.bot, PRIVATE_GROUP, t, current=price, caption=format_update(t, price))
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("closeconfirm_"):
        ...
        if not trade:
            ...
            return
        save_trades()
        await send_trade_card(context.bot, PRIVATE_GROUP, trade, current=close_price, caption=format_close(trade, close_price))
        await query.edit_message_text(
...
async def get_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_trades()
    sent = await send_trade_card(context.bot, PRIVATE_GROUP, trade, current=trade["entry"], caption=format_entry(trade))
