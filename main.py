...
async def find_best_contract(opt_type: str, spx_price: float):
    ...
    try:
        ...
        # نبحث في نطاق واسع حول SPX الحقيقي
        try:
            from ib_insync import Index
            spx_index = Index('SPX', 'CBOE')
            q = await ib.qualifyContractsAsync(spx_index)
            if q:
                t = ib.reqMktData(q[0], "", False, False)
                await asyncio.sleep(1.5)
                real_price = float(t.last or t.bid or t.ask or spx_price)
            else:
                real_price = spx_price
        except:
            real_price = spx_price

        logger.info(f"=== Search: {opt_type} real_SPX={real_price} Date={today} ===")
