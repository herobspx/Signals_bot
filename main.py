...
async def get_ibkr_price(symbol: str, expiry_str: str, opt_type: str, strike: float):
    """Get real-time option price from IB Gateway via REST API"""
    try:
        ...
        base = f"http://{IBKR_HOST}:{IBKR_PORT}/v1/api"

        async with httpx.AsyncClient(timeout=15, verify=False) as c:
            # Search for contract
            search_url = f"{base}/iserver/secdef/search"
            r = await c.post(search_url, json={"symbol": symbol, "secType": "OPT"})
            if r.status_code != 200:
                logger.error(f"IBKR search failed: {r.status_code}")
                return None
            ...
            conid = None
            for res in results:
                if res.get("companyName", "").upper().startswith(symbol.upper()):
            ...
            strikes_url = f"{base}/iserver/secdef/strikes"
            params = {"conid": conid, "sectype": "OPT", "month": dt.strftime("%b%y").upper()}
            r2 = await c.get(strikes_url, params=params)

            # Get market data snapshot
            # Build option conid
            chain_url = f"{base}/iserver/secdef/info"
            params2 = {
                "conid": conid,
                "sectype": "OPT",
            ...
            r3 = await c.get(chain_url, params=params2)
            if r3.status_code == 200:
                info = r3.json()
                if isinstance(info, list) and info:
                    opt_conid = info[0].get("conid")
                    if opt_conid:
                        # Get market data
                        snap_url = f"{base}/iserver/marketdata/snapshot"
                        r4 = await c.get(snap_url, params={"conids": opt_conid, "fields": "31,84,86"})
                        if r4.status_code == 200:
                            snap = r4.json()
                            if snap:
                                last  = snap[0].get("31", 0)  # last price
                                bid   = snap[0].get("84", 0)  # bid
                                ask   = snap[0].get("86", 0)  # ask
                                price = float(last or bid or 0)
                                if price > 0:
