...
import pytz
from trade_card import build_trade_card
...
async def send_trade_card_or_text(bot, chat_id, trade, current_price=None, caption_text=None):
    """Send the approved trade card. If image generation fails, send text fallback."""
    try:
        path = build_trade_card(trade, current_price=current_price)
        with open(path, "rb") as photo:
            return await bot.send_photo(chat_id=chat_id, photo=photo)
    except Exception as e:
        logger.exception(f"Trade card failed: {e}")
        if caption_text:
            return await bot.send_message(chat_id=chat_id, text=caption_text, parse_mode="Markdown")
        return None
                        ...
                        for ev in data:
                            if ev.get("ev") == "T" and ev.get("sym") == ticker:
                                ...
                                if abs(price - t.get("last_price", t["entry"])) >= 0.01:
                                    ...
                                    save_trades()
                                    await send_trade_card_or_text(app.bot, PRIVATE_GROUP, t, current_price=price, caption_text=format_update(t, price))
            ...
            try:
                ...
                if price:
                    ...
                    if t:
                        active_trades[trade_key]["last_price"] = price
                        save_trades()
                        await send_trade_card_or_text(
                            app.bot,
                            PRIVATE_GROUP,
                            t,
                            current_price=price,
                            caption_text=format_update(t, price),
                        )
                else:
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("closeconfirm_"):
        ...
        if not trade:
            ...
            return
        save_trades()
        await send_trade_card_or_text(context.bot, PRIVATE_GROUP, trade, current_price=close_price, caption_text=format_close(trade, close_price))
        await query.edit_message_text(
...
async def get_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_trades()
    sent = await send_trade_card_or_text(context.bot, PRIVATE_GROUP, trade, current_price=trade["entry"], caption_text=format_entry(trade))
    if sent:
