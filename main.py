...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يبحث في عقود SPXW اليوم ويختار أفضل عقد:
    - سعره بين MIN_PRICE و MAX_PRICE
    - أعلى OI + Volume
    """
    try:
        ...
        ib = IB()
        await ib.connectAsync(IBKR_HOST, IBKR_PORT, clientId=20, timeout=20)

        # نولّد strikes حول سعر SPX الحالي (ATM ± 150 نقطة)
        base = round(spx_price / 5) * 5
        strikes = [base + i * 5 for i in range(-30, 31)]
        ...
        for strike in strikes:
            try:
                ...
                candidates.append({
                    ...
                    "oi":     int(oi),
                    "vol":    int(vol),
                    "score":  int(oi) + int(vol),
                })
                logger.info(f"Candidate: {strike} mid={mid:.2f} OI={oi} Vol={vol}")
        ...
        if not candidates:
            logger.warning("No candidates found in price range")
            return None

        # الأفضل = أعلى OI + Vol
        best = max(candidates, key=lambda x: x["score"])
        logger.info(f"Best contract: {best}")
        return best
