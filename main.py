...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    V41 — اختيار عقد SPXW بطريقة أوسع وأوضح:
    ...
    try:
        ...
        ib = IB()
        await ib.connectAsync(IBKR_HOST, IBKR_PORT, clientId=41, timeout=20)
        ...
        logger.info(
            f"=== IBKR V41 Search: {opt_type} | SPX={spx_price} | Expiry={expiry} | "
            ...
            for strike in strikes:
                ...
                try:
                    variants = [
                        ...
                        Option("SPX", expiry, float(strike), right, "CBOE",  currency="USD", tradingClass="SPXW", multiplier="100"),
                        Option("SPXW", expiry, float(strike), right, "CBOE", currency="USD", multiplier="100"),
                    ]
        ...
        logger.info(
            f"=== V41 Search done. checked={checked} qualified={qualified_count} "
        ...
        logger.error(f"find_best_contract V41 error: {e}")
        return None
