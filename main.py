...
def build_daily_outlook_text(price=None, call_flow=0, put_flow=0):
    """
    يولد منشور النظرة اليومية تلقائيًا.
    مبدئيًا يستخدم السعر الحالي لتقدير الدعم والمقاومة والاستهداف.
    لاحقًا نقدر نربطه بتدفق CALL/PUT الفعلي.
    """
    if price and price > 0:
        base = round(price / 10) * 10
        support_low = int(base - 20)
        support_high = int(base - 10)
        resistance_low = int(base + 20)
        resistance_high = int(base + 30)
    else:
        support_low = 6638
        support_high = 6650
        resistance_low = 6688
        resistance_high = 6700

    mid = (support_high + resistance_low) / 2
    ...
    if call_flow > put_flow:
        ...
        bias = "CALL" if (not price or price >= mid) else "PUT"
...
async def send_daily_outlook(app, force=False):
    ...
    price = await get_polygon_spx_price()
    msg = build_daily_outlook_text(price=price)

    await app.bot.send_message(chat_id=PUBLIC_CHANNEL, text=msg)
    OUTLOOK_SENT_DATE = today_key
    logger.info(f"✅ Daily BAMSPX outlook sent | price={price}")
    return True
...
async def outlook_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != ADMIN_ID:
        return
    await send_daily_outlook(context.application, force=True)
    await update.message.reply_text("✅ تم إرسال نظرة BAMSPX اليومية للقناة العامة.")
...
async def main():
    ...
    print(f"✅ Bot v38 + Daily Outlook running on port {PORT}")
