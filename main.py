...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يجيب كل عقود SPXW 0DTE المتاحة ويختار:
    - سعر Ask أو Last بين 1.00 و 3.90
    - أقرب سعر لـ 3.90 من تحت (أعلى Delta)
    - إذا ما وجد يرسل رسالة تنبيه
    """
    MAX_PRICE = 3.90
    MIN_PRICE = 1.00
    ...
    try:
        ...
        # نبحث في strikes بخطوة 5 نقاط حول سعر SPX
        ...
        for strike in strikes:
            try:
                ...
                # السعر المرجعي: Ask أولاً ثم Last
                price = ask if ask > 0 else last
                if price <= 0:
                    continue

                logger.info(f"Strike {strike}: bid={bid} ask={ask} last={last} price={price}")
                ...
                if MIN_PRICE <= price <= MAX_PRICE:
                    ...
                    logger.info(f"✅ Candidate: {strike} price={price:.2f}")
        ...
        # اختر الأقرب لـ 3.90 من تحت (أعلى سعر ضمن النطاق)
        best = max(candidates, key=lambda x: x["price"])
        logger.info(f"✅ Best: {best['strike']} price={best['price']}")
        return best
