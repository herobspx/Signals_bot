...
# السر المشترك بين المؤشر والبوت
...
async def get_ibkr_price(symbol: str, expiry_str: str, opt_type: str, strike: float):
    try:
        ...
        if price:
            logger.info(f"IBKR price: ${price:.2f}")
            return float(price)
...
async def track_price(app, trade_key):
    logger.info(f"Auto tracking started: {trade_key}")
    while trade_key in active_trades:
        try:
            ...
            if price and price > active_trades[trade_key].get("last_price", trade["entry"]):
                ...
                save_trades()
                await send_trade_card(app.bot, PRIVATE_GROUP, active_trades[trade_key], current_price=price, caption=None)
                logger.info(f"Auto update sent: {trade_key} @ ${price:.2f}")

        except Exception as e:
            logger.error(f"Track price error {trade_key}: {e}")
...
async def manual_price_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for k, t in active_trades.items():
            ...
            save_trades()
            await send_trade_card(update.get_bot(), PRIVATE_GROUP, active_trades[k], current_price=price, caption=None)
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_trade":
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔴 PUT", callback_data="type_PUT"), InlineKeyboardButton("🟢 CALL", callback_data="type_CALL")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]])
        await query.edit_message_text("اختر نوع الصفقة:", reply_markup=kb)
        return

    if data == "menu_signal":
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔴 PUT", callback_data="signal_PUT"), InlineKeyboardButton("🟢 CALL", callback_data="signal_CALL")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]])
        await query.edit_message_text("اختر نوع الإشارة:", reply_markup=kb)
        return

    if data == "menu_trades":
        if not active_trades:
            await query.edit_message_text("📋 لا يوجد عقود نشطة.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
            return
        lines = ["📊 *العقود النشطة:*\n"]
        for k, t in active_trades.items():
        ...
        await query.edit_message_text("\n".join(lines), parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
        return
    ...
    if data == "menu_close":
        if not active_trades:
            await query.edit_message_text("لا يوجد عقود نشطة.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
            return
        buttons = [[InlineKeyboardButton(f"❌ {t['symbol']} {t['type']} | ${t['last_price']:.2f}", callback_data=f"close_{k}")] for k, t in active_trades.items()]
    ...
    if data.startswith("close_"):
        ...
        await query.edit_message_text(
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ إغلاق بـ ${trade['last_price']:.2f}", callback_data=f"closeconfirm_{trade_key}_{trade['last_price']}")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_close")]])
        )
        return
    ...
    if data.startswith("closeconfirm_"):
        ...
        save_history()
        save_trades()
        await query.edit_message_text(f"✅ تم إغلاق العقد بسعر ${close_price:.2f}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 القائمة", callback_data="menu_back")]]))
        return
    ...
    if data in ("menu_pause", "menu_resume"):
        ...
        await query.edit_message_text(status, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(toggle_lbl, callback_data=toggle_cb), InlineKeyboardButton("✏️ تحديث يدوي", callback_data="menu_manual")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
        return
    ...
    if data == "menu_manual":
        ...
        await query.edit_message_text("✏️ أرسل السعر الجديد:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
        return
    ...
    if data in ("menu_add5", "menu_add10"):
        ...
        for k, t in list(active_trades.items()):
            ...
            save_trades()
            await send_trade_card(context.bot, PRIVATE_GROUP, active_trades[k], current_price=new_price, caption=None)
        await query.answer(f"✅ +{int(add*100)}¢", show_alert=False)
        return

    if data == "menu_report":
        await query.edit_message_text("📊 اختر نوع التقرير:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📅 يومي", callback_data="report_daily")],[InlineKeyboardButton("📆 أسبوعي", callback_data="report_weekly")],[InlineKeyboardButton("🗓 شهري", callback_data="report_monthly")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
        return
    ...
    if data in ("report_daily", "report_weekly", "report_monthly"):
        ...
        trades = [t for t in closed_trades_all if datetime.fromisoformat(t.get("closed_at","1970-01-01")).replace(tzinfo=ET_TZ) >= since]
        if not trades:
            await query.edit_message_text("لا توجد صفقات.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]]))
            return
    ...
    if data.startswith("signal_"):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📣 نشر في القناة العامة", callback_data=f"pub_{sig_id}"), InlineKeyboardButton("❌ تجاهل", callback_data=f"ign_{sig_id}")]])
    ...
    if data.startswith("pub_"):
        ...
        await query.edit_message_text(f"✅ تم النشر — {signal['type']}")
...
async def trade_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"📅 اليوم ({today})", callback_data="expiry_today")],[InlineKeyboardButton("✏️ تاريخ آخر", callback_data="expiry_other")],[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]])
...
# ============================================================
# TRADINGVIEW WEBHOOK
# يستقبل تنبيه المؤشر وينشر الرسالة مباشرة في القناة الخاصة
# JSON المتوقع من المؤشر:
# {
#   "secret": "ai-candle-123",
#   "source": "BAMSPX",
#   "type": "CALL" أو "PUT",
#   "message": "نص الرسالة"
# }
...
async def handle_webhook(request):
    try:
        ...
        if data.get("secret") != WEBHOOK_SECRET:
            logger.warning("Webhook: wrong secret")
        ...
        logger.info(f"TradingView alert received: {signal_type}")
        ...
        logger.info(f"Alert sent to private group: {message[:60]}")
...
def make_stats_image(trades: list, label: str = "اليومي") -> io.BytesIO:
    ...
    GREEN=(0,218,102); RED=(232,48,48); GOLD=(222,178,0); DIV=(55,55,62)
    ...
    for ri,t in enumerate(trades):
        ...
        sign="+" if is_win else "-"
        vals=[(str(t.get("strike","")),WHITE,"bold"),(t.get("type",""),tc,"bold"),(f"${entry:.2f}",GRAY1,"mono"),(f"${maxp:.2f}",WHITE,"mono"),(f"{sign}${abs(pnl):.0f}",pc,"bold")]
    ...
    for i,(lbl,val,col,bgc) in enumerate([
        ...
        ("Net P&L",f"{'+'if net>=0 else''}{net:.0f}$",GREEN if net>=0 else RED,(8,50,20,220) if net>=0 else (55,8,8,220)),
    ]):
...
async def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            CONTRACT:   [CallbackQueryHandler(pick_expiry, pattern="^expiry_"), MessageHandler(filters.TEXT & ~filters.COMMAND, get_expiry_text)],
