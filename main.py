...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يبحث في عقود SPXW 0DTE ويختار أول عقد سعره قريب من 3.90 أو 3.80
    بغض النظر عن Strike أو OI
    """
    try:
        ...
        # SPXW 0DTE strikes بخطوة 25 نقطة
        base    = round(spx_price / 25) * 25
        strikes = [base + i * 25 for i in range(-20, 21)]

        # الأسعار المستهدفة بالترتيب
        target_prices = [3.90, 3.80, 3.85, 3.75, 3.70, 3.65, 3.60, 3.55, 3.50, 3.45, 3.40]
        tolerance     = 0.08  # تسامح ±8 سنت

        found = None
        logger.info(f"=== IBKR Search: {opt_type} | SPX={spx_price} | Date={today} | Strikes={len(strikes)} ===")
        ...
        for strike in strikes:
            try:
                ...
                logger.info(f"Strike {strike}: bid={bid} ask={ask} last={last} mid={mid}")

                if mid <= 0:
                    # جرب last price إذا ما في bid/ask
                    if last > 0:
                        mid = last
                        logger.info(f"Strike {strike}: using last={last}")
                    else:
                        continue

                # هل السعر قريب من أحد الأسعار المستهدفة؟
                for tp in target_prices:
                    if abs(mid - tp) <= tolerance:
                        found = {
                            "strike": strike,
                        ...
                        logger.info(f"✅ FOUND: {strike} mid={mid:.2f} (target {tp})")
                        break
        ...
        ib.disconnect()
        logger.info(f"=== Search done. Found: {found} ===")

        if not found:
            logger.warning("No contract found near target prices")
        return found
    ...
    try:
        ...
        if not best:
            ...
            msg = (
                ...
                f"━━━━━━━━━━━━━━━━\n"
                f"⚠️ لم يُعثر على عقد بالنطاق المحدد (3.40-3.90)\n"
                f"🎯 الهدف: {target}\n"
                f"❌ الوقف: {stop_loss}"
            )
