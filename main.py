...
async def get_ibkr_snapshot(symbol, expiry_str, opt_type, strike):
    try:
        ...
        ticker = ib.reqMktData(contracts[0], "100,101,104", False, False)
        await asyncio.sleep(2)
        ...
        snapshot = {
            "bid": px(ticker.bid),
            "ask": px(ticker.ask),
            "mid": px((ticker.bid + ticker.ask) / 2) if ticker.bid and ticker.ask else px(ticker.last),
            "open": px(ticker.open),
            "high": px(ticker.high),
            "low": px(ticker.low),
            ...
            "oi": int(
                ...
                ticker.callOpenInterest or ticker.putOpenInterest
            ) else 0
        }
    ...
    while trade_key in active_trades:
        try:
            ...
            if price:
                ...
                if snapshot:
                    active_trades[trade_key].update(snapshot)
