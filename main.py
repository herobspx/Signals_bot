...
# IBKR أحيانًا يرجع أسعار SPXW مضروبة ×1000، مثل 1440 بدل 1.44.
def normalize_option_price(value):
    try:
...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    V43 — اختيار عقد SPXW بطريقة أوسع وأوضح:
    ...
    try:
        ...
        logger.info(
            f"=== IBKR V43 Search: {opt_type} | SPX={spx_price} | Expiry={expiry} | "
        ...
        for md_mode in market_data_modes:
            ...
            for strike in strikes:
                ...
                try:
                    ...
                    bid = normalize_option_price(ticker.bid)
                    ask = normalize_option_price(ticker.ask)
                    last = normalize_option_price(ticker.last)
                    close_px = normalize_option_price(ticker.close)
                    market_price = normalize_option_price(ticker.marketPrice())
                    ...
                    price = normalize_option_price(raw_price)
        ...
        logger.info(
            f"=== V43 Search done. checked={checked} qualified={qualified_count} "
        ...
        logger.error(f"find_best_contract V43 error: {e}")
        return None
...
async def get_ibkr_price(symbol, expiry_str, opt_type, strike):
    try:
        ...
        ib.disconnect()
        normalized = normalize_option_price(price)
