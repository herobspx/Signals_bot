...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    اختيار عقد SPXW من IBKR بشرط السعر لا يتجاوز MAX_PRICE.

    إصلاحات V40:
    - IBKR غالبًا يعرّف عقود SPXW كـ symbol=SPX مع tradingClass=SPXW، وليس symbol=SPXW فقط.
    - البحث صار على سترايكات كل 5 نقاط بدل 25 نقطة حتى لا يفوّت العقود المناسبة.
    - لا يرشح أي عقد أعلى من 3.90.
    - يفضّل أعلى سعر تحت 3.90 لأنه غالبًا أقوى حركة من العقود الأرخص.
    """
    try:
        ...
        logger.info(
            f"=== IBKR V40 Search: {opt_type} | SPX={spx_price} | Date={today} | "
            ...
            try:
                ...
                for contract in contract_variants:
                    try:
                        ...
                        logger.info(f"Qualify failed strike={strike} variant={contract.symbol}/{contract.exchange}: {qe}")

                if not qualified or not used_contract:
                    logger.info(f"Strike {strike}: NOT QUALIFIED")
                    continue
                ...
                # للشراء نعتمد ask أولًا، ثم mid، ثم last.
                price = ask if ask > 0 else (mid if mid > 0 else last)
                price = round(float(price), 2) if price and price > 0 else 0

                logger.info(
                    f"Strike {strike}: bid={bid} ask={ask} last={last} mid={mid} selected_price={price} "
                    f"localSymbol={getattr(used_contract, 'localSymbol', '')}"
                )
                ...
                # لا نمنع العقود الرخيصة جدًا بشكل صارم، لكن نخلي الحد الأدنى قابل للتعديل من Render.
                if price < MIN_PRICE:
                    continue
                ...
                candidate = {
                    "symbol": "SPXW",
                    "strike": float(strike),
                    "expiry": datetime.now(ET_TZ).strftime("%d%b%y"),
                    "mid": price,
                ...
                # الأفضل: أعلى سعر تحت 3.90. عند التعادل، الأقرب لـ SPX الحالي.
                if best is None:
                    best = candidate
                else:
                    current_price = float(candidate["mid"])
                    best_price = float(best["mid"])
                    current_distance = abs(float(candidate["strike"]) - float(spx_price))
                    best_distance = abs(float(best["strike"]) - float(spx_price))

                    if current_price > best_price or (current_price == best_price and current_distance < best_distance):
                        best = candidate
        ...
        logger.info(
            f"=== V40 Search done. checked={checked} qualified={qualified_count} "
            f"priced={priced_count} under_cap={under_cap_count} best={best} ==="
        )

        if not best:
            logger.warning(
                f"No contract found <= {MAX_PRICE}. checked={checked}, qualified={qualified_count}, "
                f"priced={priced_count}, under_cap={under_cap_count}"
            )
