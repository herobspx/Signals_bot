...
IBKR_PORT       = int(os.environ.get(“IBKR_PORT”, “4001”))  # ← تم التصحيح من 4002 إلى 4001
...
“”“Get real-time option price using ib_insync — 24/7”””
try:
...
```
    ...
    if tickers:
        ...
        if price and float(price) > 0:
            logger.info(f"IBKR price: ${price}")
            return float(price)
except Exception as e:
...
```
    ...
    async with httpx.AsyncClient(timeout=15) as c:
        ...
        if r.status_code == 200:
            ...
            if price and float(price) > 0:
                logger.info(f"CBOE price: ${price}")
                return float(price)
...
“”“Get option price from Polygon — fallback”””
try:
...
day.get(“last”) or 0)
if price and float(price) > 0:
logger.info(f”Polygon snapshot: ${price}”)
return float(price)
except Exception as e:
logger.error(f”Polygon snapshot error: {e}”)
...
# ─── Track Price — IBKR أولاً ثم Polygon — بدون قيد وقت السوق ────────────────

async def track_price(app, trade_key):
“”“Auto price tracking — IBKR real-time 24/7, Polygon as fallback”””
...
```
        # 1️⃣ جرب IBKR أولاً — real-time 24/7
        ...
        # 2️⃣ إذا فشل IBKR، جرب Polygon كـ fallback
        if not price:
            logger.warning(f"IBKR failed for {trade_key}, trying Polygon...")
        ...
        # 3️⃣ أرسل التحديث إذا تغير السعر
        if price and price != active_trades[trade_key].get("last_price"):
...
“”“اختبار الاتصال بـ IBKR — افتح /test_ibkr في المتصفح”””
try:
...
ib.disconnect()
return web.Response(text=f”✅ IBKR متصل | Host: {IBKR_HOST}:{IBKR_PORT} | الحسابات: {accounts}”)
except Exception as e:
return web.Response(text=f”❌ فشل الاتصال بـ IBKR\nHost: {IBKR_HOST}:{IBKR_PORT}\nالخطأ: {str(e)}”)
...
if data in ("report_daily", "report_weekly", "report_monthly"):
    ...
    if not trades:
        await query.edit_message_text(
            "لا توجد صفقات في الفترة المحددة.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="menu_back")]])
        )
        return
...
await update.message.reply_text(“⚠️ رقم غير صحيح، مثال: `7050`”, parse_mode=“Markdown”)
return TARGET
...
trade_key = f”{d[‘symbol’]}*{d[‘strike’]}*{d[‘type’]}_{d[‘expiry’]}”
trade = {
...
last = closed_trades_today[-1]
return web.Response(text=_json.dumps({“last_closed”: last}, ensure_ascii=False), content_type=“application/json”)
...
await app.bot.send_photo(chat_id=PRIVATE_GROUP, photo=img, caption=“📊 *تقرير نهاية اليوم*”, parse_mode=“Markdown”)
...
try:
    ...
    _config = arabic_reshaper.ArabicReshaper(configuration={
        ...
        'ALEF_MADDA': True, 'ALEF_HAMZA_ABOVE': True,
        'ALEF_HAMZA_BELOW': True, 'ALEF_WASLA': True,
    })
...
def _font(size):
    for p in ["Cairo-Bold.ttf", "/app/Cairo-Bold.ttf"]:
        try: return _IFont.truetype(p, size)
        except: pass
    return _IFont.load_default()

PAD=24; ROW_H=52; HEADER=85; COL_H=40; FOOTER=105
...
f_title=_font(22); f_sub=_font(13); f_head=_font(13)
f_data=_font(14); f_sum_l=_font(13); f_sum_v=_font(13)

WHITE=(255,255,255); GREEN=(0,218,102); RED=(232,48,48)
GOLD=(222,180,0); DIV=(55,55,62)
...
for ri,t in enumerate(trades):
    ...
    draw=ImageDraw.Draw(img)
    entry=float(t.get("entry",0)); max_price=float(t.get("max_price",entry))
...
conv = ConversationHandler(
    ...
    states={
        CONTRACT:   [
            CallbackQueryHandler(pick_expiry, pattern="^expiry_"),
...
web_app.router.add_get("/test_ibkr", test_ibkr)  # ← اختبار الاتصال بـ IBKR
