...
async def send_daily_outlook(app, force=False):
    ...
    if not latest_market_outlook:
        logger.warning("⚠️ Daily outlook skipped: no MARKET_OUTLOOK data from indicator")
        return False

    msg = build_daily_outlook_text(outlook=latest_market_outlook)
    if not msg:
    ...
    logger.info(f"✅ Daily BAMSPX outlook sent from indicator data | {latest_market_outlook}")
    return True
...
async def main():
    ...
    print(f"✅ Bot v39 Outlook From Indicator running on port {PORT}")
