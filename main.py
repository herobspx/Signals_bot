...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يبحث في عقود SPXW 0DTE ويختار أفضل عقد سعره بين 1.00 و 3.90
    """
    MAX_PRICE = 3.90
    MIN_PRICE = 1.00
    ...
    try:
        ...
        # جيب سعر SPX الحقيقي من IBKR
        try:
            from ib_insync import Index
            spx_index = Index('SPX', 'CBOE')
            qualified_spx = await ib.qualifyContractsAsync(spx_index)
            if qualified_spx:
                spx_ticker = ib.reqMktData(qualified_spx[0], "", False, False)
                await asyncio.sleep(1.5)
                real_price = spx_ticker.last or spx_ticker.bid or spx_ticker.ask or spx_price
                real_price = float(real_price)
                logger.info(f"Real SPX from IBKR: {real_price}")
            else:
                real_price = spx_price
        except Exception as e:
            logger.warning(f"Could not get SPX price: {e}")
            real_price = spx_price

        base    = round(real_price / 5) * 5
        # للـ CALL نبحث فوق SPX، للـ PUT نبحث تحت SPX
        if right == "C":
            strikes = [base + i * 5 for i in range(0, 100)]   # فوق ATM
        else:
            strikes = [base - i * 5 for i in range(0, 100)]   # تحت ATM

        candidates = []
        logger.info(f"=== Search: {opt_type} SPX={spx_price} Date={today} ===")
        ...
        for strike in strikes:
            try:
                ...
                # IBKR يرجع سعر العقد مضروب في 100 — نقسم للحصول على السعر الحقيقي
                real_option_price = round(price / 100, 2)
                logger.info(f"Strike {strike}: bid={bid} ask={ask} real_price={real_option_price}")

                if MIN_PRICE <= real_option_price <= MAX_PRICE:
                    exp_dt = datetime.strptime(today, "%Y%m%d")
                    candidates.append({
                        "strike": strike,
                        "expiry": exp_dt.strftime("%d%b%y"),
                        "mid":    round(((bid + ask) / 2) / 100, 2) if bid and ask else real_option_price,
                        "bid":    round(bid / 100, 2),
                        "ask":    round(ask / 100, 2),
                        "price":  real_option_price,
                        "oi":     0,
                        "vol":    0,
                    })
                    logger.info(f"✅ Candidate: strike={strike} price={real_option_price}")
        ...
        logger.info(f"✅ Best: strike={best['strike']} price={best['price']}")
        return best
    ...
    try:
        ...
        if not best:
            ...
            msg = (
                ...
                f"━━━━━━━━━━━━━━━━\n"
                f"⚠️ لم يُعثر على عقد مناسب تحت 3.90\n"
                f"🎯 الهدف: {target}\n"
                f"❌ الوقف: {stop_loss}"
            )
