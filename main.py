                        ...
                        for ev in data:
                            if ev.get("ev") == "T" and ev.get("sym") == ticker:
                                ...
                                if abs(price - t.get("last_price", t["entry"])) >= 0.01:
                                    ...
                                    save_trades()
                                    try:
                                        card_path = build_trade_card(t, current_price=price)
                                        with open(card_path, "rb") as photo:
                                            await app.bot.send_photo(chat_id=PRIVATE_GROUP, photo=photo)
                                    except Exception as e:
                                        logger.error(f"Card send error: {e}")
...
async def get_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_trades()
    try:
        card_path = build_trade_card(trade, current_price=trade["entry"])
        with open(card_path, "rb") as photo:
            sent = await context.bot.send_photo(chat_id=PRIVATE_GROUP, photo=photo)
    except Exception as e:
        logger.error(f"Card send error: {e}")
