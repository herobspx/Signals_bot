...
# يمنع تكرار منشور النظرة اليومية في نفس اليوم
...
async def get_polygon_spx_price():
    """
    يجلب سعر SPX تقريبي من Polygon.
    إذا لم تتوفر بيانات المؤشر يستخدم SPY * 10 كبديل تقريبي.
    """
...
def build_daily_outlook_text(outlook=None, call_flow=0, put_flow=0):
    """
    يولد منشور النظرة اليومية من بيانات المؤشر أولاً.
    إذا توفر تدفق IBKR، يرجح CALL/PUT بناء عليه.
    """
...
async def get_ibkr_flow_bias(price=None):
    """
    يقرأ تدفق CALL / PUT من IBKR حول السعر القادم من المؤشر.
    يعتمد على Volume + Open Interest حول أقرب سترايكات.
    يرجع: (bias, call_flow, put_flow)
    """
    try:
        ...
        for strike in strikes:
            for right in ("C", "P"):
                try:
                    ...
                    # نعطي الحجم وزن أعلى من OI لأن الحجم يعبر عن نشاط اليوم
                    score = (vol * 3) + oi
...
async def send_daily_outlook(app, force=False):
    ...
    if not latest_market_outlook:
        logger.warning("⚠️ Daily outlook skipped: no MARKET_OUTLOOK / MARKET_DATA from indicator")
        return False
    ...
    # إذا كان IBKR أعطى Bias واضح، نستخدمه داخل النص
    ...
    logger.info(
        f"✅ Daily BAMSPX outlook sent | bias={flow_bias} call_flow={call_flow} put_flow={put_flow} | data={latest_market_outlook}"
    )
    return True

async def daily_outlook_loop(app):
    """
    يرسل النظرة اليومية تلقائيًا الساعة 4:00 مساءً بتوقيت السعودية.
    """
    while True:
        try:
            now_ksa = datetime.now(KSA_TZ)
...
async def outlook_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """اختبار يدوي من التليجرام: /outlook"""
    if update.effective_user.id != ADMIN_ID:
        return

    if not latest_market_outlook:
        await update.message.reply_text("⚠️ لم تصل بيانات النظرة من المؤشر حتى الآن. اضبط تنبيه MARKET_OUTLOOK من TradingView ثم جرّب مرة ثانية.")
        return

    sent = await send_daily_outlook(context.application, force=True)
    if sent:
        await update.message.reply_text("✅ تم إرسال نظرة BAMSPX من بيانات المؤشر للقناة العامة.")
    else:
...
async def find_best_contract(opt_type: str, spx_price: float):
    ...
    try:
        ...
        logger.info(f"=== Search: {opt_type} SPX={real_price} Date={today} ===")
        ...
        for strike in strikes:
            try:
                ...
                if MIN_PRICE <= price <= MAX_PRICE:
                    ...
                    logger.info(f"✅ Candidate: {strike} price={price} OI={oi} Vol={vol}")
        ...
        logger.info(f"✅ Best: strike={best['strike']} price={best['price']} score={best['score']}")
        return best
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── النظرة العامة من بيانات المؤشر + تدفق IBKR ──
    if data == "menu_outlook":
        if not latest_market_outlook:
            ...
            await query.edit_message_text(
                ...
                "تأكد أن تنبيه MARKET_DATA / MARKET_OUTLOOK من TradingView يعمل ثم جرّب مرة ثانية.",
...
async def main():
    ...
    # تشغيل النظرة اليومية التلقائية الساعة 4:00 مساءً بتوقيت السعودية
    ...
    print(f"✅ Bot v40 Outlook Button + IBKR Flow running on port {PORT}")
