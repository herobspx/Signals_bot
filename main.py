    ...
    try:
        ...
        for k, t in active_trades.items():
            ...
            await send_trade_card(
                ...
                active_trades[k],
                current_price=price,
                caption=format_update(active_trades[k], price)
            )
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("closeconfirm_"):
        ...
        save_history()
        save_trades()
        await send_trade_card(context.bot, PRIVATE_GROUP, trade, current_price=close_price, caption=format_close(trade, close_price))
        await query.edit_message_text(
    ...
    if data in ("menu_add5", "menu_add10"):
        ...
        for k, t in list(active_trades.items()):
            ...
            await send_trade_card(
                ...
                caption=format_update(active_trades[k], new_price)
            )
...
def make_stats_image(trades_history: list) -> io.BytesIO:
    ...
    # Column headers
    headers = ["العقد", "سعر الدخول", "أعلى سعر", "الربح/الخسارة"]
    x = 0
...
def make_stats_image(trades: list, label: str = "اليومي") -> io.BytesIO:
    ...
    f_sub   = ImageFont.truetype(BOLD, 13)
    f_head  = ImageFont.truetype(BOLD, 13)
    f_data  = ImageFont.truetype(BOLD, 14)
    f_sum_l = ImageFont.truetype(BOLD, 13)
    f_sum_v = ImageFont.truetype(BOLD, 13)
    ...
    draw.text((PAD+10, HEADER//2 - 12), f"تقرير SPX  ·  {label}", fill=GOLD, font=f_title, anchor="lm")
    draw.text((PAD+10, HEADER//2 + 16), f"{days_ar[now.weekday()]}  ·  {now.strftime('%d %b %Y')}", fill=WHITE, font=f_sub, anchor="lm")
    draw.line([0, HEADER, W, HEADER], fill=DIV, width=1)

    # Column headers
    img = rect_alpha(img, [0, HEADER, W, HEADER+COL_H], (6,6,10,220))
    draw = ImageDraw.Draw(img)
    headers = ["العقد", "النوع", "سعر الدخول", "أعلى سعر", "الربح / الخسارة"]
    ...
    for i,(lbl,val,col,box_bg) in enumerate([
        ("إجمالي الربح",   f"+${total_profit:.0f}", GREEN, (8,40,18)),
        ("إجمالي الخسارة", f"-${abs(total_loss):.0f}", RED, (44,8,8)),
        ("صافي الربح",     f"{'+'if net>=0 else''}${net:.0f}", GREEN if net>=0 else RED, (8,40,18) if net>=0 else (44,8,8)),
    ]):
