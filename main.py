...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يجيب قائمة العقود المتاحة من IBKR مباشرة
    ثم يختار أفضل عقد سعره بين 1.00 و 3.90
    """
    MAX_PRICE = 3.90
    MIN_PRICE = 1.00
    ...
    try:
        ...
        # جيب قائمة العقود المتاحة من IBKR
        ...
        # اختر أول chain
        chain = chains[0]
        logger.info(f"Chain: expirations={list(chain.expirations)[:5]} strikes_count={len(chain.strikes)}")

        # اختر تاريخ اليوم
        if today not in chain.expirations:
            # جرب أقرب تاريخ متاح
            available = sorted(chain.expirations)
            today = available[0] if available else today
            logger.warning(f"Today not in expirations, using: {today}")

        # فلتر الـ strikes حول سعر SPX الحالي
        all_strikes = sorted(chain.strikes)
        nearby = [s for s in all_strikes if abs(s - spx_price) <= 500]
        logger.info(f"Nearby strikes: {len(nearby)}")
        ...
        for strike in nearby:
            try:
                ...
                logger.info(f"Strike {strike}: bid={bid} ask={ask} last={last}")
                ...
                if MIN_PRICE <= price <= MAX_PRICE:
                    ...
                    logger.info(f"✅ Candidate: strike={strike} price={price:.2f}")
