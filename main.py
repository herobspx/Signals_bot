...
async def find_best_contract(opt_type: str, spx_price: float):
    """
    يبحث في عقود SPXW 0DTE ويختار أفضل عقد لا يتجاوز MAX_PRICE.
    المنطق الجديد:
    - لا يشترط نطاق 3.40 - 3.90.
    - يقبل أي عقد سعره بين MIN_PRICE و MAX_PRICE.
    - يفضل أعلى سعر تحت 3.90 لأنه غالبًا أقوى دلتا من العقود الأرخص.
    - سعر الشراء المعتمد: ask إذا موجود، ثم last، ثم mid.
    """
    try:
        ...
        base = round(spx_price / 25) * 25
        strikes = [base + i * 25 for i in range(-30, 31)]

        best = None
        logger.info(
            f"=== IBKR Search: {opt_type} | SPX={spx_price} | Date={today} | "
            f"Allowed price={MIN_PRICE}-{MAX_PRICE} | Strikes={len(strikes)} ==="
        )
        ...
        for strike in strikes:
            try:
                ...
                # لأننا مشترين، الأفضل نعتمد ask. إذا ما توفر، نستخدم last، ثم mid.
                price = ask if ask > 0 else (last if last > 0 else mid)
                ...
                # لا ترشح عقد أغلى من 3.90، ولا عقد ضعيف جدًا تحت الحد الأدنى.
                ...
                # اختر أعلى سعر تحت 3.90، ومع التعادل اختر الأقرب لـ ATM.
                if best is None:
                    best = candidate
                else:
                    current_price = float(candidate["mid"])
                    best_price = float(best["mid"])
                    current_distance = abs(float(candidate["strike"]) - float(base))
                    best_distance = abs(float(best["strike"]) - float(base))
        ...
        ib.disconnect()
        logger.info(f"=== Search done. Best under {MAX_PRICE}: {best} ===")
