...
async def get_webull_price(symbol: str, expiry_str: str, opt_type: str, strike: float):
    if not WEBULL_EMAIL or not WEBULL_PASSWORD:
        return None
    try:
        import hashlib
        ...
        pwd = md5("wl_" + md5(WEBULL_PASSWORD))
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "Mozilla/5.0",
            "did": "webull_bot_001",
            "hl": "en", "os": "web", "ver": "3.40.14",
            "app": "webull", "appid": "webull-ajax",
        }
        login_payload = {
            "account": WEBULL_EMAIL,
            "accountType": "2",
            "pwd": pwd,
            "deviceId": "webull_bot_001",
            "grade": 1,
            "regionId": 6,
        }
        async with httpx.AsyncClient(timeout=15) as c:
            ...
            if r.status_code != 200:
                logger.error(f"Webull login failed: {r.status_code}")
                return None
            ...
            params = {
                ...
                "direction": "put" if opt_type.upper() == "PUT" else "call",
                "strikePriceBegin": str(int(strike) - 10),
                "strikePriceEnd": str(int(strike) + 10),
            }
            r2 = await c.get(
                "https://userapi.webull.com/api/options/chain/strikes",
            ...
            if r2.status_code == 200:
                ...
                for item in items:
                    if abs(float(item.get("strikePrice", 0)) - strike) < 1.0:
                        for field in ["latestPriceVo", "close", "last"]:
                            val = item.get(field)
                            if isinstance(val, dict):
                                price = float(val.get("close", 0) or val.get("last", 0))
                            elif val:
                                price = float(val)
                            else:
                                continue
                            if price > 0:
                                logger.info(f"Webull price: ${price}")
                                return price
    except Exception as e:
        logger.error(f"Webull price error: {e}")
    return None

async def get_price_rest(ticker, symbol="", expiry="", opt_type="", strike=0):
    if symbol and expiry and opt_type and strike:
        price = await get_webull_price(symbol, expiry, opt_type, strike)
        if price:
            return price
        logger.info("Webull failed, trying Polygon...")
    try:
