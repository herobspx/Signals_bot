...
def make_stats_image(trades_history: list) -> io.BytesIO:
    ...
    # Column headers
    headers = [ar("العقد"), ar("سعر الدخول"), ar("أعلى سعر"), "الربح/الخسارة"]
    x = 0
...
def make_stats_image(trades: list, label: str = "اليومي") -> io.BytesIO:
    ...
    draw.text((PAD+10, HEADER//2 - 12), ar(f"تقرير SPX  ·  {label}"), fill=GOLD, font=f_title, anchor="lm")
    draw.text((PAD+10, HEADER//2 + 16), f"{ar(days_ar[now.weekday()])}  ·  {now.strftime('%d %b %Y')}", fill=WHITE, font=f_sub, anchor="lm")
    draw.line([0, HEADER, W, HEADER], fill=DIV, width=1)

    # Column headers
    img = rect_alpha(img, [0, HEADER, W, HEADER+COL_H], (6,6,10,220))
    draw = ImageDraw.Draw(img)
    headers = [ar("العقد"), ar("النوع"), ar("سعر الدخول"), ar("أعلى سعر"), ar("الربح / الخسارة")]
    ...
    for i,(lbl,val,col,box_bg) in enumerate([
        (ar("إجمالي الربح"),   f"+${total_profit:.0f}", GREEN, (8,40,18)),
        (ar("إجمالي الخسارة"), f"-${abs(total_loss):.0f}", RED, (44,8,8)),
        (ar("صافي الربح"),     f"{'+'if net>=0 else''}${net:.0f}", GREEN if net>=0 else RED, (8,40,18) if net>=0 else (44,8,8)),
    ]):
