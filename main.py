...
def make_card(trade: dict, current_price: float, card_type: str = "update") -> io.BytesIO:
    """Generate professional trade card image"""
    from PIL import Image, ImageDraw, ImageFont
    ...
    is_put  = trade["type"].upper() == "PUT"
    h_color = RED if is_put else GREEN
    draw.rounded_rectangle([20, 20, W-20, 72], radius=16, fill=h_color)
    draw.rectangle([20, 52, W-20, 72], fill=h_color)

    if card_type == "entry":   header = f"{'🔴 PUT' if is_put else '🟢 CALL'}  |  دخول جديد"
    elif card_type == "close": header = f"{'🔴 PUT' if is_put else '🟢 CALL'}  |  إغلاق العقد"
    else:                      header = f"{'🔴 PUT' if is_put else '🟢 CALL'}  |  تحديث"

    draw.text((W//2, 46), header, fill=WHITE, font=font_med, anchor="mm")
    draw.text((W//2, 100), f"{trade['symbol']}  ${trade['strike']}  {trade['expiry']}  {trade['type'].upper()}", fill=GOLD, font=font_med, anchor="mm")
    draw.line([40, 122, W-40, 122], fill=BORDER, width=1)

    entry  = float(trade["entry"])
    diff   = current_price - entry
    pct    = (diff / entry) * 100 if entry else 0
    sign   = "+" if diff >= 0 else ""
    pc     = GREEN if diff >= 0 else RED

    draw.text((60, 140), "سعر الدخول", fill=GRAY, font=font_tiny)
    draw.text((60, 160), f"${entry:.2f}", fill=WHITE, font=font_med)
    draw.text((60, 205), "السعر الحالي", fill=GRAY, font=font_tiny)
    draw.text((60, 225), f"${current_price:.2f}", fill=WHITE, font=font_med)

    draw.text((W//2+20, 140), "ربح / خسارة", fill=GRAY, font=font_tiny)
    draw.text((W//2+20, 160), f"{sign}${diff:.2f}", fill=pc, font=font_med)
    draw.text((W//2+20, 205), "النسبة", fill=GRAY, font=font_tiny)
    draw.text((W//2+20, 225), f"{sign}{pct:.1f}%", fill=pc, font=font_med)

    draw.line([40, 270, W-40, 270], fill=BORDER, width=1)
    draw.text((60, 285), "🎯 الهدف", fill=GRAY, font=font_small)
    draw.text((60, 308), str(trade.get("target", "-")), fill=GREEN, font=font_small)
    draw.text((W//2+20, 285), "❌ وقف الخسارة", fill=GRAY, font=font_small)
    draw.text((W//2+20, 308), str(trade.get("stop", "-")), fill=RED, font=font_small)

    draw.line([40, 348, W-40, 348], fill=BORDER, width=1)
    draw.text((W//2, 370), datetime.now(ET_TZ).strftime("%Y-%m-%d  %H:%M ET"), fill=GRAY, font=font_tiny, anchor="mm")
    draw.text((W//2, 400), "BAM Signals", fill=BORDER, font=font_small, anchor="mm")
